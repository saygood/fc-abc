...
parser.add_argument('--job_dir', type=str, default='./experiments/cifar/resnet56/bee_search',
...
parser.add_argument('--honey_model', type=str,
                    default='/home/chengyijie/pycharm/ABCPruner/experiments/Training/vgg/1e-0599s.pth.tar',
...
parser.add_argument('--max_preserve', type=int, default=4, help='Minimum percent of training per layer')
parser.add_argument('--pr', type=float, default=0.78, help='pruning ratio of model')
...
parser.add_argument('--honeychange_num', type=int, default=3,
...
food_scale_cfg ={
'resnet56': [16, 32, 64] + [16] * 9 + [32] * 9 + [64] * 9,
}
...
ckpt = torch.load(args.honey_model)  # honey_model?????
...
def calculationFitnessold(honey, train_loader, args):
    global best_honey
    ...
    if args.arch == 'resnet_cifar':
        model = import_module(f'model.{args.arch}').resnet_new(args.cfg, honeysource=honey).to(device)
    ...
    for i in range(args.food_number):
        ...
        while Flag:
            ...
            for j in range(honeychange_num):
                ...
                if EmployedBee[i].code[param2change[j]] > args.max_preserve * (2 ** (log(args.food_scale[param2change[j]], 2) - log(args.food_scale[0], 2))):
                    EmployedBee[i].code[param2change[j]] = int(args.max_preserve * (2 ** (log(args.food_scale[param2change[j]], 2) - log(args.food_scale[0], 2))))
            if is_legal(EmployedBee[i].code):
                break
            else:
                honeychange_num -= 1
        ...
        if (R_choosed < NectraSource[i].rfitness):
            ...
            while True:
                ...
                for j in range(honeychange_num):
                    ...
                    if OnLooker[i].code[param2change[j]] > args.max_preserve * 2 ** (log(args.food_scale[param2change[j]], 2) - log(args.food_scale[0], 2)):

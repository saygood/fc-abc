...
class PatchEmbedding(nn.Module):
    def __init__(self, patch_size, hidden_size, drop_prob=DROP_PROB):
        ...
        self.proj = nn.Linear(dim, hidden_size)#定义一个线性层
        self.drop = nn.Dropout(drop_prob)#以一定的比例dropout
...
class PatchEmbedding_naspruner(nn.Module):
    def __init__(self, patch_size, hidden_size, drop_prob= DROP_PROB):
        super().__init__()
    ...
    def forward(self, x):
        ...
        #随机扔掉一定量的数据点，对于图像而言，几个点对预测结果影响不大
...
class MSA(nn.Module):
    ...
    def _get_attention_score(self, q, k):
        # "$qk^{T}$"   b:batch size n: query length? m:key length? h:number of heads d:dimension per head 
...
class MSA_nas(nn.Module):
    def __init__(self, hidden_size, n_heads, drop_prob= DROP_PROB):
        ...
        self.qkv_proj = nn.Linear(hidden_size, 3 * n_heads * self.head_size, bias=False)
        self.drop = nn.Dropout(drop_prob)
        self.out_proj = nn.Linear(n_heads * self.head_size, hidden_size, bias=False)
    ...
    def forward(self, x):
        # "$[q, k, v] = zU_{qkv}$"
        q, k, v = torch.split(self.qkv_proj(x), split_size_or_sections=self.n_heads * self.head_size, dim=2,)
...
class TransformerEncoderLayer_nas(nn.Module):
    def __init__(self, hidden_size, mlp_size, n_heads):
        super().__init__()

        self.self_attn = MSA_nas(hidden_size=hidden_size, n_heads=n_heads)
        self.self_attn_resid = SkipConnection(hidden_size=hidden_size)
        self.mlp = MLP(hidden_size=hidden_size, mlp_size=mlp_size)
        self.mlp_resid = SkipConnection(hidden_size=hidden_size)
...
class TransformerEncoder_nas(nn.Module):
    def __init__(self, n_layers, hidden_size, mlp_size, nl_heads):
        super().__init__()

        self.enc_stack = nn.ModuleList(
            [TransformerEncoderLayer_nas(hidden_size=hidden_size, mlp_size=mlp_size, n_heads=nl_heads[i])
...
class ViT_new(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(torch.randn((1, n_patches + 1, hidden_size)))
        self.drop1 = nn.Dropout(drop_prob)
        self.tf_enc = TransformerEncoder_nas(
            n_layers=n_layers, hidden_size=hidden_size, mlp_size=mlp_size, nl_heads=nl_heads,
        )

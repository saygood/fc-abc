...
class DenseBasicBlock(nn.Module):
    def __init__(self, inplanes, filters, index, expansion=1, growthRate=12, dropRate=0):
        ...
        # [36, 48, 60, 72, 84, 96, 108, 120, 132, 144, 156, 168, 180, 192, 204, 216, 228, 240, 252, 264, 276, 288, 300, 312, 324, 336, 348, 360, 372, 384, 396, 408, 420, 432, 444, 456]
...
class DenseNet(nn.Module):
    ...
    def __init__(self, depth=40, block=DenseBasicBlock,
        dropRate=0, num_classes=100, growthRate=12, compressionRate=2, filters=None, honey=None, indexes=None):
        ...
        if filters == None:
            filters = []
            start = growthRate*2 #24
            index = 0
            for i in range(3):
                index -= 1
                filter = 0
                for j in range(n+1):#dense40: n=12
                    if j != 0 :
...
class DenseNet_new(nn.Module):
    ...
    def __init__(self, depth=40, block=DenseBasicBlock,
        ...
        self.food_scale = [24, 12, 12, 12]
        assert (depth - 4) % 3 == 0, 'depth should be 3n+4'
        n = (depth - 4) // 3 if 'DenseBasicBlock' in str(block) else (depth - 4) // 6
        self.cell = 1
        if honey is None:
            self.honey = [24, 12, 12, 12]
        else:
            self.honey = honey
        ...
        if filters == None:
            filters = []
            start = self.honey[0] #24
            for i in range(3):
                
                filter = 0
                for j in range(n+1):#dense40: n=12
                    if j != 0 :
                        filter += self.honey[i+1]
                    filters.append([start + filter])                  
                start = (start + self.honey[i+1] * n) // compressionRate
    ...
    def _make_denseblock(self, block, blocks, filters, indexes):
        ...
        for i in range(blocks):
            # print("denseblock inplanes", filters[i])
            self.growthRate = self.honey[self.currentindex]
            if i != 0:
                self.inplanes += self.growthRate                  
            layers.append(block(self.inplanes, filters=filters[i], index=indexes[i], growthRate=self.growthRate, dropRate=self.dropRate))   
        self.inplanes += self.growthRate
...
def densenet_new(honey=None, **kwargs):
    return DenseNet_new(depth=40, block=DenseBasicBlock, compressionRate=1, honey=honey, **kwargs)

def test():
    honey = [12, 4, 4, 4] 
    model = densenet_new(honey = honey)

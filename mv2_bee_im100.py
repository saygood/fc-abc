...
parser.add_argument('--gpus', type=int, nargs='+', default=[2], help='Select gpu_id to use. default:[0]', )
...
parser.add_argument('--data_path', type=str, default='/home/yijiechen/imagenet/imagenet100/Data/',help='The dictionary where the input is stored. default:', )
parser.add_argument('--job_dir', type=str, default='./experiments/Training/mv2_im100/searching', help='The directory where the summaries will be stored. default:./experiments', )
...
parser.add_argument('--honey_model', type=str, default='/home/yijiechen/workspace/ABCPruner/experiments/Training/mv2_im100/training/checkpoint/model_best.pt',
...
parser.add_argument('--pr', type=float, default=0.3, help='pruning ratio of model')
...
args.traindir = os.path.join(args.data_path, 'train')
args.valdir =  os.path.join(args.data_path, 'val')
...
def set_loader(args):
    ...
    train_dataset, batch_size=args.train_batch_size, shuffle=True, num_workers=12, pin_memory=True)
    val_loader = torch.utils.data.DataLoader(
    val_dataset, batch_size=args.eval_batch_size, shuffle=False, num_workers=12, pin_memory=True)
...
if args.dataset == 'cifar10':
    ...
    trainLoader,testLoader = set_loader(args)
...
if args.arch == 'vgg_cifar':
    ...
    origin_model = import_module(f'model.{args.arch}').mobilenetv2_100().to(device)

if args.honey_model is None or not os.path.exists(args.honey_model):
    raise ('Honey_model path should be exist!')

ckpt = torch.load(args.honey_model, map_location='cpu')
# new_ckpt = {}
...
# Training
def train(model, optimizer, trainLoader,args, epoch, topk=(1,5)):
    ...
    for batch, (images, labels) in enumerate(trainLoader):
        ...
        accuracy.update(prec1[0], inputs.shape[0])
        top5_accuracy.update(prec1[1], inputs.shape[0])
        ...
        if (batch + 1) % print_freq == 0:
            ...
            logger.info(
                ...
                'Top5 {:.2f}%\t'
                'Time {:.2f}s'.format(
                    epoch, batch * args.train_batch_size, float(losses.avg), float(accuracy.avg), float(top5_accuracy.avg), cost_time
                )
            )
...
# Testing
def test(model, testLoader, topk=(1,5)):
    ...
    with torch.no_grad():
        for batch_idx, (images, labels) in enumerate(testLoader):
            ...
            accuracy.update(predicted[0], inputs.shape[0])
            top5_accuracy.update(predicted[1], inputs.shape[0])
...
def get_arch_flops(honey):
    ...
    model = import_module(f'model.{args.arch}').mobilenetv2_100(honey).to(device)
...
def calculationFitness(honey, args):
    ...
    model = import_module(f'model.{args.arch}').mobilenetv2_100(honey=honey).to(device)
    ...
    for epoch in range(args.calfitness_epoch):
        for batch, (images, labels) in enumerate(trainLoader):
            ...
            output = model(inputs)
            loss = loss_func(output, targets)
            prec1 = utils.accuracy(output, targets, topk=(1,5))
        ...
        current_time = time.time()
        cost_time = current_time -start_time
        print('Epoch[{}]:\t Time {:.2f}s'.format(epoch, cost_time))
        start_time = current_time
...
def main():
    ...
    if args.resume == None:
        ...
        if args.arch == 'vgg_cifar':
            ...
            model = import_module(f'model.{args.arch}').mobilenetv2_100(honey=best_honey.code).to(device)
        ...
        if args.arch == 'vgg_cifar':
            ...
            model = import_module(f'model.{args.arch}').mobilenetv2_100(honey=code).to(device)
    ...
    if args.test_only:
        ...
        for epoch in range(start_epoch, args.num_epochs):
            ...
            #         param_group['lr'] *= 0.1
            train(model, optimizer, trainLoader, args, epoch, topk=(1, 5) if args.dataset == 'imagenet' else (1,))
